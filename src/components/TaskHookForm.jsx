    ...
    <form className="taskForm" onSubmit={handleSubmit(handleSubmitHandler)}>
      ...
      <div className="form-line">
        ...
        <textarea
          ...
          rows="3"
          id="description"
          {...register("description",{required: "Task açıklaması yazmalısınız", minLength:{
            value: 10, 
        ...
        <div>
          {kisiler.map((p) => (
            <label className="input-checkbox" key={p}>
              <input
                type="checkbox"
                {...register("people", {required:"Lütfen en az bir kişi seçin", validate: {maxPeople: p=> p.length <= 3 || "En fazla 3 kişi seçebilirsiniz"}})}
                value={p}
